...
def token_merge_bipartite2d(
    ...
    with torch.no_grad():
        # Keep protected indices as tensors in both modes to simplify typing.
        ...
        # Define internal split functions with fixed return signatures for type checkers.
        def split_src_dst(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            C = x.shape[-1]
            src = gather(x, dim=1, index=a_idx.expand(B, num_src, C))
            dst = gather(x, dim=1, index=b_idx.expand(B, num_dst, C))
            return src, dst

        def split_src_dst_protected(
            x: torch.Tensor,
        ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
            C = x.shape[-1]
            src = gather(x, dim=1, index=a_idx.expand(B, num_src, C))
        ...
        if enable_protection:
            a, b, _ = split_src_dst_protected(metric)
        else:
            a, b = split_src_dst(metric)
    ...
    def merge(
        ...
        protected = x[..., :0, :]
        if enable_protection:
            src, dst, protected = split_src_dst_protected(x)
        else:
            src, dst = split_src_dst(x)
        ...
        if extra_tensors is not None:
            ...
            protected_e = extra_tensors[..., :0, :]
            if enable_protection:
                src_e, dst_e, protected_e = split_src_dst_protected(extra_tensors)
            else:
                src_e, dst_e = split_src_dst(extra_tensors)
        ...
        if extra_tensors_2 is not None:
            ...
            protected_e2 = extra_tensors_2[..., :0, :]
            if enable_protection:
                src_e2, dst_e2, protected_e2 = split_src_dst_protected(extra_tensors_2)
            else:
                src_e2, dst_e2 = split_src_dst(extra_tensors_2)
